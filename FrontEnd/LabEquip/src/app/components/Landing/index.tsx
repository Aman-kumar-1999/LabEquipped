...
//   useEffect(() => {
//     if (items.length === 0) {
//       dispatch(fetchProducts({ page: 0, size: 6, sortBy: "productName", sortDir: "ASC" }));
//     }
//   }, [dispatch]);
...
//                     product={product}
//                     onAddToCart={() => handleAddToCart1(product.productId)}
  ...
  <ChatLauncherClient />
      ...
      <section className="l-container my-5">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <small className="text-muted">AI-powered suggestions</small>
        </div>
        <div className="row g-4">
          {aiLoading ? (
            <p>Loading suggestions...</p>
          ) : aiError ? (
            <p className="text-danger">Error: {aiError}</p>
          ) : aiSuggestionsData.length === 0 ? (
            <p>No suggestions yet.</p>
          ) : (
            aiSuggestionsData.map((p: any, idx: number) => (
              <div key={idx} className="col-12 col-md-4">
                <div className="card product-card h-100">
                  <a className="text-reset" href={`./product_details.html?sku=${p.sku || p.id || idx}`}>
                    <img alt={p.name || 'Suggested product'} className="card-img-top"
                      src={p.image || `${process.env.NEXT_PUBLIC_API_BASE_URL}/products/${p.id}/image`} />
                  </a>
                  <div className="card-body d-flex flex-column">
                    <h5 className="card-title mb-1">{p.name}</h5>
                    <p className="text-muted mb-2">{p.category || ''}</p>
                    <div className="mt-auto d-flex justify-content-between align-items-center">
                      <span className="fw-bold">${(p.price || 0).toFixed ? p.price.toFixed(2) : p.price || '0.00'}</span>
                      <button className=" x-btn x-btn--primary btn-sm add-to-cart" onClick={() => {
                        dispatch(addItem({
